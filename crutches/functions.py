...
def shift_months(months_to_add, basedate=None):
    ...
    if basedate is None: basedate = date.today()
    years_to_add, month = divmod(basedate.month + months_to_add, 12)
    if month == 0: month = 12
    return date(basedate.year + years_to_add, month, 1) + timedelta(days=basedate.day-1)
...
def attribs(object, as_unicode=False):
    ...
    return dict((key, unicode(value) if as_unicode else value) for key, value in object.__dict__.iteritems() if not callable(value) and not key.startswith('__'))
...
def int_str(number, base=None, digits="0123456789abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ"):
    ...
    if base < 2 or base > len(digits):
        raise NotImplementedError, "The base must be between 2 and {0}.".format(len(digits))
...
def int_base(source, base=None, digits="0123456789abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ"):
    """ The equivalent to int(), but the base may go up to 62 (allowing for uppercase letters). """
    if base is None:
        base = len(digits)
    if base < 2 or base > 62:
        raise NotImplementedError, "int_base() base must be >= 2 and <= {0}.".format(len(digits))
    result = 0
    step = len(source)
    for char in source:
        step = step - 1
