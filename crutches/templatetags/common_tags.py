...
def iconize(type, size=32, set='crystal', default_type='application/octet-stream'):
    ...
    return '<img src="{0}/{1}/{2}?{3}" alt="" />'.format(home, set, type, urlencode(params).replace('&', '&amp;'))
...
def flatpage_link(url, additional_class = False):
    try:
        ...
        if additional_class:
            return '<a href="{0}" class="{1}"><span>{2}</span></a>'.format(flatpage.url, additional_class, flatpage.title)
        else:
...
class ExtractValue(template.Node):
    ...
    def render(self, context):
        ...
        if actual_dict[context[self.index]]:
            ...
            raise template.TemplateSyntaxError, "Context variable %s does not contain index %s!" % (self.dict_name, self.index)
